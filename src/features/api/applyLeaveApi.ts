...
import { ApplyLeaveResponse, GetAllLeaveApplicationsByDateResponse, GetAllLeaveApplicationsByPaginationResponse, GetAllLeaveApplicationsResponse } from "@/types";
...
export const applyLeaveApi = createApi({
    ...
    endpoints: (builder) => ({
        ...
        getAllLeaveApplicationsByStatus: builder.query<GetAllLeaveApplicationsResponse, {
            status: string,
            type?: string
            startDate?: string
            endDate?: string,
            ...
            query: ({ status, type, startDate, endDate, search, page = 0, size = 10, sort = 'desc' }) => {
                let queryString = `/leave-applications/status/${status}?`;
                if (type) {
                    queryString += `leaveType=${type}&`;
                }
                if (startDate) {
        ...
        getLeaveApplicationByDate: builder.query<GetAllLeaveApplicationsByDateResponse, { date: string, department: string }>({
